...
def extract_features(data_file_name, model_path, batch_size, embedding_size):
	...
	with tf.Graph().as_default(), tf.device('/gpu:1'):
		...
		try:
			...
			y = graph.get_tensor_by_name('prelogits:0')
...
if __name__ == '__main__':
	embeddings, label = extract_features(data_file_name='../data/1_4/test.txt', model_path='../model/20190104-151227/arc_fault-38160', 
		batch_size=100, embedding_size=25)

	np.savetxt('../data/1_4/test_features.txt', np.column_stack((embeddings, label)), fmt='%.6f')

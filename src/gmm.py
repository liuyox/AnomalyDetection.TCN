...
if __name__ == '__main__':
	dataset = np.loadtxt('../data/1_4/train_features.txt')
	...
	test_data = np.loadtxt('../data/1_4/train_features.txt')
	...
	tpr, fpr, F1, accuracy, best_threshold_index = calc_roc(thresholds, scores, np.logical_xor(label, 1))
	...
	#for point in negative_examples:
	color = 'b'
	plt.scatter(negative_examples[:,0], negative_examples[:, 1], 30,c=color,marker='o',linewidths=0.75,edgecolors='k')
	...
	X, Y = np.meshgrid(x, y)
	XX = np.column_stack([X.reshape(-1), Y.reshape(-1)])
	prob = np.exp(clf.score_samples(XX))
	prob = prob.reshape(X.shape)[::-1] / max(prob)
	np.savetxt('prob.txt', prob, fmt='%.6f')
	plt.imshow(prob)
	plt.colorbar()
	plt.show()
	exit(0)
	...
	for i in range(n_components):
		prob = temp[:,i].reshape(X.shape)[::-1,:]
		plt.figure()
		plt.imshow(prob)
		#np.savetxt('prob_{}.txt'.format(i+1), prob, fmt='%.6f')
		#CS = plt.contour(X, Y, temp[:,i].reshape(X.shape))
		plt.colorbar()
	...
	#fig.set_size_inches(4, 4)
	#plt.savefig('gmm_figure_arc.png', format='png')

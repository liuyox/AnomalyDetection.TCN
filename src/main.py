...
def train(args):
    ...
    with tf.Graph().as_default():
        ...
        net = Network(args.tcn_num_channels, args.tcn_kernel_size, args.tcn_dropout, 
            args.embedding_size, args.weight_decay, args.num_classes)
        ...
        with tf.variable_scope('optimizer'), tf.control_dependencies([centers_update_op]):
            ...
            optimizer = tf.train.AdamOptimizer(args.lr_values, name='optimizer')
        ...
        with tf.Session(config=config) as sess:
            train_writer = tf.summary.FileWriter(log_dir, sess.graph, filename_suffix='train')
                ...
                for epoch in range(1, args.max_epochs+1):
                    ...
                    for step in batch_num_seq:
                        ...
                        if step % args.display == 0:
                            tensor_list = [train_op, train_summary_op, global_step, accuracy_op, tpr_op,
                            ...
                            _, summary, train_step = sess.run([train_op, train_summary_op, global_step], feed_dict={
                                                              x: feature, y: label, is_training: True})
                        train_writer.add_summary(summary, global_step=train_step)
                        train_writer.flush()
                    ...
                    for i in range(num_batches):
                        ...
                        feature = np.reshape(feature, (args.batch_size, args.timesteps, 1))
                        tensor_list = [accuracy_op, tpr_op, fpr_op, g_mean_op, val_summary_op]
                        feed_dict = {x: feature, y: label, is_training: False}
                        accuracy_array[i], tpr_array[i], fpr_array[i], g_mean_array[i], summary = sess.run(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tcn_num_channels', type=list, default=[25, 25, 25], help='')
    parser.add_argument('--tcn_kernel_size', type=int, default=8, help='')
    parser.add_argument('--tcn_dropout', type=float, default=0.1, help='')
    parser.add_argument('--num_classes', type=int, default=2, help='')
    parser.add_argument('--embedding_size', type=int, default=2, help='')
    ...
    parser.add_argument('--lr_values', type=float, default=0.01, help='')
    parser.add_argument('--display', type=int, default=50, help='')

    parser.add_argument('--center_loss_factor',type=float,default=0.1,help='center loss ratio in total loss.')
